...
void Game::DrawStatus(ClientCard* pcard, int x1, int y1, int x2, int y2) {
	...
	if(pcard->type & TYPE_LINK) {
		...
		myswprintf(textBuffer, L"☆%d", pcard->lscale);
		w = adFont->getDimension(textBuffer).Width;
		DrawShadowText(textFont, textBuffer, Resize(x1 + 5, y1 + 1, x1 + 5, y1 + 21, 0, 0, w, 0), Resize(1, 1, 1, 1),
...
void Game::ShowElement(irr::gui::IGUIElement * win, int autoframe) {
	...
	for(auto fit = fadingList.begin(); fit != fadingList.end(); ++fit)
		if(win == fit->guiFading && win != wOptions && win != wANNumber) // the size of wOptions is  setted by ClientField::ShowSelectOption before showing it

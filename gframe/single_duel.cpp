...
void SingleDuel::UpdateDeck(DuelPlayer* dp, void* pdata, unsigned int len) {
	...
	if(duel_count == 0) {
		deck_error[dp->type] = deckManager.LoadDeck(pdeck[dp->type], (int*)deckbuf, mainc,extrac, sidec , false, true);
	} else {
...
int SingleDuel::Analyze(unsigned char* msgbuffer, unsigned int len) {
	...
	while (pbuf - msgbuffer < (int)len) {
		...
		case MSG_FLIPSUMMONING: {
			BufferIO::ReadInt32(pbuf);
			RefreshSingle(BufferIO::ReadInt8(pbuf), BufferIO::ReadInt16(pbuf), BufferIO::ReadInt8(pbuf));
			pbuf += 1;
			NetServer::SendBufferToPlayer(players[0], STOC_GAME_MSG, offset, pbuf - offset);
			NetServer::ReSendToPlayer(players[1]);
			for(auto oit = observers.begin(); oit != observers.end(); ++oit)
...
void SingleDuel::RefreshSingle(int player, int location, int sequence, int flag) {
	...
	NetServer::SendBufferToPlayer(players[player], STOC_GAME_MSG, query_buffer, len + 5);
	if (len <= LEN_HEADER)
		return;
	const int clen = BufferIO::ReadInt32(qbuf);
	auto position = GetPosition(qbuf, 8);
	...
	NetServer::SendBufferToPlayer(players[1 - player], STOC_GAME_MSG, query_buffer, len + 5);
	for (auto pit = observers.begin(); pit != observers.end(); ++pit)
		NetServer::ReSendToPlayer(*pit);
}
